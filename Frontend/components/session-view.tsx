...
//   );
// };
// make sure file is in /public/lottie
...
export const SessionView = ({
  ...
  // --------- Conversation Tracking + Analysis Hooks ---------
  useEffect(() => {
    if (messages.length > 0) {
      const latestMessage = messages[messages.length - 1];
      const isFromUser = latestMessage.from?.isLocal ?? false;
      ...
      setConversationHistory((prev) => {
        const exists = prev.some((msg) => msg.message === latestMessage.message);
        if (!exists) {
          return [
            ...prev,
            {
  ...
  return (
    <section
      ...
      inert={disabled}
      className={cn(
        'relative min-h-screen overflow-hidden bg-gradient-to-br from-slate-900 via-gray-900 to-slate-900 opacity-0',
        !chatOpen && 'max-h-svh overflow-hidden'
      )}
    >
      {/* Animated Background */}
      ...
      {/* Top Header */}
      <div className="fixed top-0 right-0 left-0 z-40 border-b border-slate-700/50 bg-slate-900/80 backdrop-blur-xl">
        <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 md:px-6">
          {/* AI Status with Siri Animation */}
          <div className="flex items-center gap-3">
            <div className="relative">
              <div
                className={cn(
                  'flex h-10 w-10 items-center justify-center overflow-hidden rounded-full transition-all duration-300',
                  agentState === 'speaking' && 'ring-2 ring-blue-400/40',
                  agentState === 'listening' && 'ring-2 ring-green-400/40',
                  agentState === 'thinking' && 'ring-2 ring-yellow-400/40',
                  'ring-1 ring-slate-600'
                )}
              >
                <Lottie animationData={siriAnimation} loop autoplay className="h-10 w-10" />
              </div>
              <div
                className={cn(
                  'absolute -right-1 -bottom-1 h-4 w-4 rounded-full border-2 border-slate-900',
                  agentState === 'speaking' && 'bg-blue-500',
                  agentState === 'listening' && 'bg-green-500',
                  agentState === 'thinking' && 'bg-yellow-500',
                  'bg-gray-500'
            ...
            <div>
              <h2 className="font-bold text-white drop-shadow-[0_0_10px_rgba(255,255,255,0.8)]">
                SWARA
              </h2>
      ...
      <div
        className={cn(
          'flex min-h-screen flex-col pb-32 transition-all duration-300',
          lastAnalysis ? 'pt-24' : 'pt-16',
          showMobileAnalysis && 'pt-32 md:pt-16'
        )}
      >
        {/* Chat Messages */}
        {chatOpen && (
          <ChatMessageView className="mx-auto w-full max-w-4xl flex-1 px-4 py-6 md:px-6">
            <div className="space-y-6">
              <AnimatePresence>
                {messages.map((message: ReceivedChatMessage) => (
                  <motion.div
        ...
        <div
          className={cn(
            'relative flex flex-1 items-center justify-center transition-all duration-500',
            chatOpen ? 'hidden' : 'block'
          )}
        >
          <MediaTiles chatOpen={chatOpen} />
      ...
      {/* Floating Control Bar */}
      <div className="fixed right-0 bottom-0 left-0 z-50">
        <div className="pointer-events-none absolute inset-0 bg-gradient-to-t from-slate-900 via-slate-900/95 to-transparent" />
        <div className="relative px-4 pt-8 pb-6 md:px-6 md:pb-8">
          <motion.div
            key="control-bar"
            initial={{ opacity: 0, translateY: '100%' }}
