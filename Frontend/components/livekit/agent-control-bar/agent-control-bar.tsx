...
export function AgentControlBar({
  ...
  return (
    <div
      aria-label="Voice assistant controls"
      className={cn(
        'flex flex-col rounded-3xl border border-slate-600/50 bg-slate-800/90 p-6 shadow-2xl shadow-black/20 backdrop-blur-xl',
        className
      )}
      {...props}
    >
      ...
      <div className="flex flex-row items-center justify-center gap-4">
        ...
        {visibleControls.microphone && (
          <div className="relative flex items-center">
            <TrackToggle
              ...
              onPressedChange={microphoneToggle.toggle}
              className={cn(
                'relative h-16 w-16 rounded-full border-2 p-0 shadow-lg transition-all duration-200 hover:scale-110 active:scale-95',
                microphoneToggle.enabled
                  ? 'border-green-400 bg-gradient-to-r from-green-500 to-emerald-600 text-white shadow-green-500/25'
                  : 'border-red-400 bg-gradient-to-r from-red-500 to-red-600 text-white shadow-red-500/25 hover:from-red-600 hover:to-red-700'
              )}
            />
            {/* Device selector */}
            <div className="absolute -right-2 -bottom-2 hidden md:block">
              <DeviceSelect
                ...
                onMediaDeviceError={onMicrophoneDeviceSelectError}
                onActiveDeviceChange={handleAudioDeviceChange}
                className="h-8 w-8 rounded-full border border-slate-500/50 bg-slate-700/90 p-1 backdrop-blur-sm transition-colors hover:bg-slate-600/90"
              >
        ...
        {capabilities.supportsVideoInput && visibleControls.camera && (
          <div className="relative flex items-center">
            <TrackToggle
              ...
              onPressedChange={cameraToggle.toggle}
              className={cn(
                'relative h-14 w-14 rounded-full border-2 p-0 shadow-lg transition-all duration-200 hover:scale-105 active:scale-95',
                cameraToggle.enabled
                  ? 'border-blue-400 bg-gradient-to-r from-blue-500 to-cyan-600 text-white shadow-blue-500/25'
                  : 'border-slate-500 bg-slate-600 text-slate-300 shadow-slate-500/25 hover:bg-slate-500'
              )}
            />
            {/* Device selector */}
            <div className="absolute -right-1 -bottom-1 hidden md:block">
              <DeviceSelect
                ...
                onMediaDeviceError={onCameraDeviceSelectError}
                onActiveDeviceChange={handleVideoDeviceChange}
                className="h-6 w-6 rounded-full border border-slate-500/50 bg-slate-700/90 p-1 backdrop-blur-sm transition-colors hover:bg-slate-600/90"
              >
        ...
        {capabilities.supportsScreenShare && visibleControls.screenShare && (
          <TrackToggle
            ...
            className={cn(
              'relative h-14 w-14 rounded-full border-2 p-0 shadow-lg transition-all duration-200 hover:scale-105 active:scale-95',
              screenShareToggle.enabled
                ? 'border-purple-400 bg-gradient-to-r from-purple-500 to-indigo-600 text-white shadow-purple-500/25'
                : 'border-slate-500 bg-slate-600 text-slate-300 shadow-slate-500/25 hover:bg-slate-500'
            )}
          >
            {screenShareToggle.enabled ? (
        ...
        {visibleControls.chat && (
          <Toggle
            ...
            className={cn(
              'relative h-14 w-14 rounded-full border-2 p-0 shadow-lg transition-all duration-200 hover:scale-105 active:scale-95',
              chatOpen
                ? 'border-indigo-400 bg-gradient-to-r from-indigo-500 to-purple-600 text-white shadow-indigo-500/25'
                : 'border-slate-500 bg-slate-600 text-slate-300 shadow-slate-500/25 hover:bg-slate-500 disabled:opacity-50'
            )}
          >
        ...
        {visibleControls.leave && (
          <Button
            ...
            onClick={onLeave}
            disabled={isDisconnecting}
            className="ml-2 h-14 w-14 rounded-full border-2 border-red-400 bg-gradient-to-r from-red-500 to-red-600 p-0 shadow-lg shadow-red-500/25 transition-all duration-200 hover:scale-105 hover:from-red-600 hover:to-red-700 active:scale-95"
          >
            {isDisconnecting ? (
