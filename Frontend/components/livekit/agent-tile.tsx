...
// const AIBot3D = dynamic(() => import("./AIBot3D").then(mod => mod.AIBot3D), { ssr: false });
...
import { Canvas } from '@react-three/fiber';
import { cn } from '@/lib/utils';
import animaBot from '@/public/Anima Bot.json';
...
//   );
// };
// ✅ put your JSON in /public/lottie
...
export const AgentTile = ({
  ...
  return (
    <div
      ref={ref}
      className={cn(
        'from-navy-900 dark:from-navy-950 relative flex flex-col items-center justify-center overflow-hidden rounded-3xl border border-white/10 bg-gradient-to-br via-indigo-900 to-slate-900 shadow-[0_8px_30px_rgb(0,0,0,0.4)] dark:via-indigo-950 dark:to-slate-950',
        className
      )}
    >
      {/* Three.js Background Scene */}
      ...
      <div className="relative z-10 flex flex-col items-center">
        ...
        <div className="relative mb-4">
          <div className="relative flex items-center justify-center">
            {state === 'speaking' && (
              <>
                <span className="absolute h-28 w-28 animate-ping rounded-full bg-blue-500/20" />
                <span className="absolute h-36 w-36 animate-pulse rounded-full bg-blue-400/10" />
              </>
            )}

            <div
              className={cn(
                'relative flex h-20 w-20 items-center justify-center rounded-full shadow-[0_0_20px_rgba(255,255,255,0.2)] transition-all duration-300 md:h-24 md:w-24',
                state === 'speaking' && 'scale-110 bg-white/10 ring-2 ring-blue-400/40',
                state === 'listening' && 'scale-105 bg-green-400/10 ring-2 ring-green-400/40',
                state === 'thinking' && 'scale-105 bg-yellow-400/10 ring-2 ring-yellow-400/40',
                'bg-white/5'
              )}
            >
              {/* ✅ Lottie animation replaces AIBot3D */}
              <Lottie
            ...
            <div
              className={cn(
                'border-navy-950 absolute -top-1 -right-1 flex h-6 w-6 items-center justify-center rounded-full border-2 shadow-md',
                state === 'speaking' && 'animate-pulse bg-blue-400',
                state === 'listening' && 'animate-pulse bg-green-400',
                state === 'thinking' && 'animate-spin bg-yellow-400',
                'bg-gray-500'
              )}
            >
              {state === 'speaking' && <Volume2 className="h-3 w-3 text-white" />}
            </div>
          </div>
        </div>
        ...
        <div className="mb-4 text-center">
          <h3 className="text-lg font-semibold tracking-wide text-white drop-shadow-lg md:text-xl">
            Swara
          </h3>
          <p className="text-sm tracking-wide text-white/70 capitalize">
            {state === 'listening' && 'Listening...'}
            {state === 'thinking' && 'Processing...'}
            {state === 'speaking' && 'Speaking...'}
            {state === 'connecting' && 'Connecting...'}
        ...
        <div>
          ...
          >
            <span
              className={cn([
                'min-h-1 w-1 rounded-full bg-gradient-to-t from-indigo-300 to-white/70 transition-all duration-200 ease-out',
                'data-[lk-highlighted=true]:from-white data-[lk-muted=true]:from-slate-500 data-[lk-muted=true]:to-slate-400/40',
              ])}
            />
          </BarVisualizer>
