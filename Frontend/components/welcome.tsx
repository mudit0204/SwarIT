...
//   );
// };
import { MessageSquare, Mic, Sparkles, Video, Zap } from 'lucide-react';
import { Button } from '@/components/ui/button';
import { cn } from '@/lib/utils';
...
export const Welcome = ({
  ...
  return (
    <section
      ref={ref}
      inert={disabled}
      className={cn('fixed inset-0 overflow-hidden bg-white', disabled ? 'z-10' : 'z-20')}
    >
      {/* Animated Background */}
      <div className="absolute inset-0">
        <div className="absolute top-1/4 left-1/4 h-96 w-96 animate-pulse rounded-full bg-purple-500/20 blur-3xl"></div>
        <div className="absolute right-1/4 bottom-1/4 h-96 w-96 animate-pulse rounded-full bg-blue-500/20 blur-3xl delay-1000"></div>
        <div className="absolute top-1/2 left-1/2 h-72 w-72 -translate-x-1/2 -translate-y-1/2 transform animate-pulse rounded-full bg-indigo-500/20 blur-3xl delay-500"></div>
      </div>
      ...
      <div className="relative z-10 flex min-h-screen flex-col items-center justify-center px-4 text-center">
        {/* Main Logo/Icon */}
        <div className="relative mb-12">
          <div className="flex h-32 w-32 rotate-3 transform items-center justify-center rounded-3xl bg-gradient-to-br from-blue-900 via-blue-600 to-blue-800 shadow-2xl shadow-blue-500/50 transition-transform duration-300 hover:rotate-6">
            <Sparkles className="h-16 w-16 text-white" />
          </div>

          {/* Floating elements */}
          <div className="absolute -top-4 -right-4 flex h-8 w-8 animate-bounce items-center justify-center rounded-full bg-gradient-to-r from-yellow-400 to-orange-500 delay-200">
            <Zap className="h-4 w-4 text-white" />
          </div>
          <div className="absolute -bottom-4 -left-4 flex h-8 w-8 animate-bounce items-center justify-center rounded-full bg-gradient-to-r from-green-400 to-blue-500 delay-700">
        ...
        {/* Title */}
        <h1
          className="via-grey-100 mb-0 bg-gradient-to-r from-black to-black bg-clip-text text-4xl leading-tight font-medium text-transparent md:text-6xl"
          style={{ fontFamily: 'poppins, serif' }}
        >
          AI Voice Assistant for <br /> Complaint Management
        </h1>
        <h1 className="mb-8 bg-gradient-to-r from-purple-400 via-pink-400 to-red-400 bg-clip-text text-3xl font-bold text-transparent md:text-5xl"></h1>

        {/* Subtitle */}
        <p className="mx-auto mb-4 max-w-2xl text-xl leading-relaxed text-black md:text-2xl">
          Experience the future of conversation with our <br /> intelligent AI assistant
        </p>

        {/* Start Button */}
        <div className="group relative">
          <div className="absolute -inset-1 animate-pulse rounded-full bg-gradient-to-r from-blue-800 via-blue-600 to-blue-900 opacity-75 blur transition duration-1000 group-hover:opacity-100 group-hover:duration-200"></div>
          <Button
            onClick={onStartCall}
            className="relative min-w-[280px] transform rounded-full border-0 bg-gradient-to-r from-blue-900 via-blue-600 to-blue-800 px-12 py-6 text-xl font-bold text-white shadow-2xl transition-all duration-200 hover:scale-105 hover:from-blue-950 hover:via-blue-700 hover:to-blue-900 active:scale-95"
          >
            <div className="flex items-center gap-3">
